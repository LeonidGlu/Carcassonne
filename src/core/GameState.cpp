...
bool GameState::placeTile(const Tile& tile, Position pos) {
	if (!validator.canPlace(tile, pos, board)) {
		DEBUG_LOG("Can't place tile on (" << pos.x << ", " << pos.y << ")");
		return false;
	}

	Tile mutableTile = tile;

	DEBUG_LOG("Place tile on (" << pos.x << ", " << pos.y << "):");
	DEBUG_CALL(mutableTile.debug());

	regionManager.onTilePlaced(mutableTile, pos, board);
	board.setTile(pos, std::move(mutableTile));
